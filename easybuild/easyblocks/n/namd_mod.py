...
#
# Copyright:: Copyright 2013-2017 CaSToRC, The Cyprus Institute
...
class EB_NAMD_MOD(MakeCp):
    ...
    def extra_options():
        ...
        extra.update({
            ...
            'charm_arch': [None, "Charm++ target architecture (see the subdirectories in the charm-x.y.z/src/arch subdirectory)", MANDATORY],
            'charm_opts': ['--with-numa --with-production', "Charm++ build options (really those that start with --, the compiler choice and so on are determined based on the toolchain and toolchain optionsi and specific needs for NAMD).", CUSTOM],
    ...
    def __init__(self,*args,**kwargs):
        ...
        super(EB_NAMD_MOD, self).__init__(*args, **kwargs)
        self.namd_arch = None

    def extract_step(self):
        """Custom extract step for NAMD, we need to extract charm++ so we can patch it."""
        super(EB_NAMD_MOD, self).extract_step()
    ...
    def configure_step(self):
        ...
        if fftw:
            # FFTW options for NAMD differ for FFTW v3 or earlier versions of FFTW.
            if LooseVersion(get_software_version('FFTW')) >= LooseVersion('3.0'):
                # Test if NAMD is version 2.9 or younger as older versions don't support FFTW v3.
    ...
    def build_step(self):
        ...
        super(EB_NAMD_MOD, self).build_step(path=self.namd_arch)
    ...
    def make_module_extra(self):
        ...
        txt = super(EB_NAMD_MOD, self).make_module_extra()
    ...
    def sanity_check_step(self):
        ...
        super(EB_NAMD_MOD, self).sanity_check_step(custom_paths=custom_paths)
