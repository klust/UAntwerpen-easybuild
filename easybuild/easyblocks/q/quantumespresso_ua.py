...
class EB_QuantumESPRESSO_UA(ConfigureMake):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(EB_QuantumESPRESSO_UA, self).__init__(*args, **kwargs)

        if LooseVersion(self.version) >= LooseVersion("6.4"):
            self.install_subdir = "q-e-qe-%s" % self.version
    ...
    def patch_step(self):
        ...
        super(EB_QuantumESPRESSO_UA, self).patch_step(beginpath=self.builddir)
    ...
    def configure_step(self):
        ...
        super(EB_QuantumESPRESSO_UA, self).configure_step()
    ...
    def sanity_check_step(self):
        ...
        if 'pp' in targets or 'pwall' in targets or 'all' in targets:
            ...
            if LooseVersion(self.version) >= LooseVersion("6.4"):
                bins.remove("bgw2pw.x")
                bins.remove("pw_export.x")
            else:
        ...
        if 'd3q' in targets:
            ...
            if LooseVersion(self.version) >= LooseVersion("6.4"):
                d3q_bins.remove("d3_import3py.x")
        ...
        super(EB_QuantumESPRESSO_UA, self).sanity_check_step(custom_paths=custom_paths)
